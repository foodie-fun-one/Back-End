...
router.get("/", verifyToken, (req, res) => {
  restModel
    ...
    .catch(err => {
      res.status(500).json({
        errorMessage:
          "An error has occured attempting to retrieve a list of restaurants."
      });
    });
});
...
router.get("/:id", verifyToken, (req, res) => {
  restModel
    ...
    .catch(err => {
      ...
      res.status(500).json({
        errorMessage:
          "An error has occured attempting to acquire information on this restaurant."
      });
    });
});
...
router.put("/:id", verifyToken, (req, res) => {
  restModel
    ...
    .then(rest => {
      restModel.findById(req.params.id).then(item => {
        if (item.length === 0) {
          res.status(404).json({
            errorMessage: "A restaurant with this ID could not be found."
          });
          ...
          if (
            ...
            res.status(400).json({
              errorMessage:
                "Please ensure all fields are included.(name, address, and hours)."
            });
          }
        }
      });
    ...
    .catch(err => {
      res.status(500).json({
        errorMessage:
          "An error occured while attempting to update this restaurant."
      });
    });
});
...
router.delete("/:id", verifyToken, (req, res) => {
  restModel
    ...
    .catch(err => {
      ...
      res.status(500).json({
        errorMessage:
          "An error occured attempting to delete the restaurant with the following ID."
      });
    });
});
